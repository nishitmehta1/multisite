	...
	color : function(target, prop) {
		...
		function drawPicker(x, y) {
			...
			if(!jscolor.picker) {
				...
				jscolor.picker.boxB.className="mpl_color_picker";
			...
			p.box.id = 'mpl-color-picker-box';
			p.pad.id = 'mpl-color-picker-pad';
			p.padM.id = 'mpl-color-picker-padM';
			p.padB.id = 'mpl-color-picker-padB';
			p.boxB.id = 'mpl-color-picker-boxB';
			p.sld.id = 'mpl-color-picker-sld';
			
			p.sldB.id = 'mpl-color-picker-sldB';
			p.sldBO.id = 'mpl-color-picker-sldBO';
			
			p.sldM.id = 'mpl-color-picker-sldM';
			p.sldO.id = 'mpl-color-picker-sldO';
			p.sldO.title = 'Opacity';
			
			p.btn.id = 'mpl-color-picker-btn';
			...
			p.btn.onclick = function (e) {
				...
				if(e.target.tagName == 'BUTTON'){
					...
					if( typeof( Storage ) !== "undefined" ){
						...
						if( localStorage['mpl_color_presets'] !== undefined )
							data = localStorage['mpl_color_presets'].split('|');
						...
						if( color === '' )
							...
							localStorage.removeItem( 'mpl_color_presets' );
							localStorage.setItem( 'mpl_color_presets', data.join('|') );
					...
					if(localStorage['mpl_color_presets'] === undefined || color === '')
						return;
					
					if (localStorage['mpl_color_presets'] === '')
						data = "#61B0FF|#9782FF|#FFD970|#CBFF63|#4DFFAC|#FF21E1|#9021FF|#1F87FF|#21FFBC|#55FF21";
					else data = localStorage['mpl_color_presets'];
			...
			p.btn.render = function(){
				...
				if( typeof( Storage ) !== "undefined" ){
					...
					if (localStorage['mpl_color_presets'] !== undefined && localStorage['mpl_color_presets'] !== '')
						data = localStorage['mpl_color_presets'].split('|');
		...
		jscolor.addEvent(target, 'focus', function() {
			if( THIS.pickerOnfocus && !document.getElementsByClassName('mpl_color_picker')[0] ) { THIS.showPicker(); }
		});		
