...
class mpl_ajax_front{
	...
	function twitter_timeline() {
		...
		switch ($display_style) {
			...
			case '2':
				$el_in_start = '<div class="mpl-tweet-owl owl-carousel owl-theme">';
				$el_in_end = '</div>';
		...
		if (
			...
			if( isset( $show_follow_button ) && $show_follow_button == 'yes' && $display_style == 1  ){
				...
				<a class="mpl_twitter_follow" href="https://twitter.com/intent/user?screen_name='. esc_attr( $username ) .'" target="_blank"><i class="fa fa-twitter"></i>Follow Us</a>
					...
					$output .= $el_item_start;
						if(!empty( $latestTweet )){
							...
							if ( isset( $show_avatar ) && $show_avatar == 'yes' ){
								//$avatar = '<span class="mpl_tweet_icon"><i class="fa fa-twitter"></i></span>';
